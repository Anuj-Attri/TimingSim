    ...
    def dispatch(self, task_input):
        ...
        if type == "mul" and len(self.MulPipeline) == 0:
            ...
            if len(busyMems) == 0:
                ...
                for idx in range(len(busyMems)):
                    # Try to add any memory to the memory bank
                    return_value = public_vmembb.InitBank(mem_addr=busyMems[idx])
                    if return_value != -1:
                        # Successfully request the mem access
                        task["cycle"] += 1
                        ...
                        self.VmemBankQ.remove(busyMems[idx])
        ...
        if len(self.MulPipeline) != 0:
            ...
            if len(self.VmemReturnQ) == 0:
                ...
                if len(self.VmemBankQ) != 0:
                    for idx in range(len(self.VmemBankQ)):
                    # Try to add any memory to the memory bank
                        return_value = public_vmembb.InitBank(mem_addr=self.VmemBankQ[idx])
                        if return_value != -1:
                            # Successfully add to memory bank
                            addr = self.VmemBankQ[idx]
                            self.VmemBankQ.remove(addr)
